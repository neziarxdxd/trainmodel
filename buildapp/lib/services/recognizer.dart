...
class Recognizer {
  Future loadModel() {
    ...
    return Tflite.loadModel(
        model: "assets/lord_thankyou.tflite", labels: "assets/labels.txt");
  }
  ...
  Future recognize(List<Offset> points) async {
    ...
    Uint8List bytes =
        await _imageToByteListUint8(picture, Constants.mKulitanSize);
  ...
  Future<Uint8List> _imageToByteListUint8(Picture pic, int size) async {
    final ui.Image img = await pic.toImage(size, size);
  ...
  Picture _pointsToPicture(List<Offset> points) {
    ...
    final canvas = Canvas(recorder, _canvasCullRect)
      ..scale(Constants.mKulitanSize / Constants.canvasSize);
